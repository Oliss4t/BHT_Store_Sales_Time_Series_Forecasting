...
These are then compared with two other models: Prophet, an algorithm by Facebook; and decision trees (regression tree and a Random Forest).
...
The first thing we did is splitting the train data into a taining and validation set.
...
We used that datastructure for all of the follwing models: Exponential Smoothing, Double Exponential Smoothing, SARIMA and the decision trees.
...
Those lags could be interesting to include in the model.
...
It produces the following prediction plot (@fig-random-forest), again not as good as the cheated version in @fig-random-forest-cheated. 
